...
function siteSize(){
  ...
  var len = document.getElementById('proj').scrollHeight;
  ...
  if(window.innerHeight >= len) {
    scroll.style.height = window.innerHeight + "px";
  } else {
    scroll.style.height = (window.innerHeight * window.innerHeight / len) + "px";
  }
}
...
function intro(){
  ...
  document.querySelector(".scrollbar").style.top = "0px";
  siteSize();
}
...
document.getElementById('proj').addEventListener("scroll", function(){
  var len = document.getElementById('proj');
  document.querySelector('.scrollbar').style.top = (len.scrollTop / len.scrollHeight) * 100 + "%";
});
var mouseID = false;
document.querySelector(".scrollbar").addEventListener('mousedown', function(){
  mouseID = true;
  var len = document.getElementById('proj');
  initY = event.clientY;
  document.addEventListener('mousemove', function() {
    if (mouseID) {
      len.scrollBy(0, (event.clientY - initY));
    }
  });
